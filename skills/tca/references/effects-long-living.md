...
This application demonstrates how to handle long-living effects (e.g. notifications from Notification Center) and how to tie an effect's lifetime to the lifetime of the view.

## Pattern Guidelines
- **Task Action**: Declare a `.task` action in the feature's Action enum.
- **View Task Lifecycle**: Bind the view lifecycle to the task using SwiftUI's `.task { await store.send(.task).finish() }`. This automatically cancels the associated asynchronous task when the view goes off-screen, preventing memory leaks and unwanted background updates.
- **AsyncSequence Iteration**: Iterate over asynchronous sequences (like notifications, timers, or sockets) using a `for await` loop within the `.run` block.
