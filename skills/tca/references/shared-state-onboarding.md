# Shared State - Onboarding

This case study demonstrates how to use shared data in order to implement a complex, multi-step onboarding/signup flow.

## Pattern Guidelines
- **Shared In-Memory Values**: Initialize an in-memory `@Shared` data model and pass it around down the stack or between screen state objects (`BasicsFeature.State(signUpData: store.$signUpData)`).
- **Binding Direct Property Access**: Derive a standard SwiftUI `@Binding` in the view from a `@Shared` property on the store to update properties easily without excessive actions (`@Binding(store.$signUpData) var signUpData`).
- **Conditional Detents**: Handle sub-features by opening sheets driven by target destinations (`$store.scope(\.$destination, action: \.destination).basics`).
