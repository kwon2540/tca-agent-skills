# Getting Started - Optional State

This demonstrates how to compose an optional child feature and render it conditionally in SwiftUI.

## Pattern Guidelines
- **Optional State Properties**: Declare child states as optional in the parent `State` struct (e.g. `var optionalChild: Child.State?`).
- **IfLet Operator**: Apply the `.ifLet(\.optionalChild, action: \.optionalChild)` operator inside the parent reducer body to handle forwarding actions to the child reducer when it is non-nil.
- **Conditional Scoping**: In the View, use `if let store = store.scope(\.optionalChild, action: \.optionalChild)` to conditionally extract a non-optional store when state is present.
