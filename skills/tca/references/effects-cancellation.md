...
This screen demonstrates how to cancel in-flight asynchronous effects in the Composable Architecture.

## Pattern Guidelines
- **Cancellation Identifier**: Declare a private enum `CancelID` to identify specific running effects (e.g. `private enum CancelID { case factRequest }`).
- **Cancellable Modifier**: Append `.cancellable(id: CancelID.factRequest)` to the end of your `.run` effect chain.
- **Cancel Effect**: Return `.cancel(id: CancelID.factRequest)` to immediately cancel any active effect with the designated identifier.
