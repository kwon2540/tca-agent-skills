...
This application demonstrates how to work with a web socket client in the Composable Architecture.

## Pattern Guidelines
- **Actor-Isolated Client**: Create a dependency client (e.g. `WebSocketClient`) that interfaces with a global-actor-isolated client (e.g. `@globalActor final actor WebSocketActor`) to manage connection resources thread-safely.
- **Multiple Async Tasks**: Use `withThrowingTaskGroup` inside `.run` to manage parallel sub-tasks associated with the socket connection lifecycle, such as continuously receiving messages and executing periodic heartbeats.
- **Task Cancellation**: Cancel socket tasks cleanly using `.cancel(id: ClientID)` to tear down connections.
