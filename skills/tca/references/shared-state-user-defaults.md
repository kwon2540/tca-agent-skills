# Shared State - User Defaults

This screen demonstrates how multiple independent screens can share state in the Composable Architecture through user defaults (app storage).

## Pattern Guidelines
- **App Storage Key**: Extend `SharedKey` where `Self == AppStorageKey<Value>` to declare a key matching a string identifier (e.g. `appStorage("sharedStateDemoCount")`).
- **State Integration**: Declare properties in your state structs with `@Shared(.count)` to automatically load/save values from UserDefaults.
- **Modification Rule**: Mutate key-bound states using `$count.withLock { $0 += 1 }` to guarantee thread safety.
