# Navigation - Multiple Destinations

This screen demonstrates driving multiple kinds of navigation (drill down, sheet, popover) from a single piece of enum state.

## Pattern Guidelines
- **Destination Reducer Enum**: Define a `@Reducer enum Destination` containing all potential screens or modal states.
- **Presents Wrapper**: Mark the active destination in your parent state with `@Presents var destination: Destination.State?`.
- **View bindings**: Use `.navigationDestination(item:)`, `.popover(item:)`, and `.sheet(item:)` tied to the scoped store of each enum case (e.g. `$store.scope(\.$destination, action: \.destination).sheet`).
