# Getting Started - Bindings Basics

This demonstrates how to handle two-way bindings in SwiftUI with TCA in a unidirectional manner using the `.sending` API.

## Pattern Guidelines
- **Unidirectional Bindings**: Derive bindings from the store by chaining a property from state and using `.sending(\.action)` to direct mutations through the reducer.
- **View Store Binding**: Annotate the store property with `@Bindable` in SwiftUI to access bindings directly: `$store.property.sending(\.action)`.
- **Keyboard Dismissal**: You can chain SwiftUI/TCA helpers like `.resignFirstResponder()` on bindings.
