# Getting Started - Bindings Forms

This demonstrates how to handle two-way bindings in forms in the Composable Architecture, eliminating boilerplate actions using `BindableAction` and `BindingReducer`.

## Pattern Guidelines
- **Bindable Action Protocol**: Conform the feature's `Action` enum to `BindableAction` and define a case `case binding(BindingAction<State>)`.
- **Binding Reducer**: Place `BindingReducer()` before your custom `Reduce` block in the reducer's body composition to automatically handle state mutations.
- **Specific Binding Mutations**: Intercept mutations of specific properties in the reducer by matching on the case path (e.g. `case .binding(\.stepCount)`).
- **SwiftUI View Integration**: Store views can bind directly to `$store.propertyName` when the store is marked with `@Bindable`.
