...
This screen demonstrates how multiple independent screens can share state in the Composable Architecture through an in-memory reference.

## Pattern Guidelines
- **Shared Key In-Memory**: Define an `InMemoryKey` (e.g. `inMemory("stats")`) to share state globally in-memory across features without disk persistence.
- **Shared Reference**: Use `@Shared(.stats)` to declare state that synchronizes instantly when mutated in any sub-features.
- **Lock-protected Mutations**: Mutate shared state via `$stats.withLock { ... }` in reducers.
