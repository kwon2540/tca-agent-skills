...
This screen demonstrates how to introduce side effects (like network calls and timers) into a feature built with the Composable Architecture.

## Pattern Guidelines
- **Effect.run**: Return `.run` from the reducer to execute asynchronous, throwing, or long-running work in a concurrent context.
- **Action Callback**: Within `.run`, use the provided `send` parameter to send actions back to the store when asynchronous work completes (e.g. `await send(.numberFactResponse(...))`).
- **Cancellation**: Register an effect as cancellable using `.cancellable(id:)` so it can be manually cancelled using `.cancel(id:)`.
