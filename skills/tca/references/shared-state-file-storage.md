...
This screen demonstrates how multiple independent features can share state in the Composable Architecture through file storage.

## Pattern Guidelines
- **Shared State Key**: Define a custom `SharedKey` mapping to file storage using the `.fileStorage` key type.
- **Shared Property Wrapper**: Declare a property using `@Shared(.stats) var stats = Stats()` to load/save the persisted value.
- **Lock-protected Modifications**: Always mutate shared values using the `$stats.withLock { ... }` block to ensure thread-safe modifications.
- **Synchronization**: Since multiple child features reference `@Shared(.stats)`, changes made in one are automatically propagated to other features sharing that key.
