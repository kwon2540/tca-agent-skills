...
## Pattern Guidelines
- **Timer Sequence**: Use the `.timer(interval:)` extension on Swift Clocks (e.g. `self.clock.timer(interval: .seconds(1))`) to iterate over clock ticks asynchronously.
- **cancellable(cancelInFlight: true)**: When restarting or toggling a timer, use `.cancellable(id: CancelID.timer, cancelInFlight: true)` to automatically tear down the previous timer instance if it was already running.
- **Tear down on disappearance**: Send a teardown action (like `.onDisappear`) in SwiftUI's `.onDisappear` view modifier to clean up active timer effects when the view is popped or dismissed.
