...
This screen demonstrates how to use `NavigationStack` with Composable Architecture applications to push and pop multiple features using `StackState`.

## Pattern Guidelines
- **StackState Definition**: Define the stack state property as `var path = StackState<Path.State>()` where `Path` is a nested `@Reducer enum` representing every destination screen in the stack.
- **StackAction Definition**: Define the corresponding action as `case path(StackActionOf<Path>)`.
- **forEach Reducer Composition**: Add `.forEach(\.path, action: \.path)` to the reducer's body composition to integrate child feature state changes and delegate actions.
- **View Stack Integration**: Bind the stack in SwiftUI using `NavigationStack(path: $store.scope(\.path, action: \.path)) { RootView() } destination: { store in switch store.case { ... } }`.
- **2.0-ready Effects**: Sequence work inside `.run`; do not append deprecated `Effect.concatenate` or scheduler-based operators.
