# Navigation - Lists - Navigate & Load

This screen demonstrates navigation that depends on loading optional state from a list element.

## Pattern Guidelines
- **Optional Selection State**: Model selected rows in your State using `Identified<ID, ChildState?>?` to represent an optional sub-feature whose state is loaded asynchronously.
- **Scope Scoping**: In the View, scope to the optional nested value inside selection: `store.scope(\.selection?.value, action: \.counter)`.
- **Double ifLet Reducer Composition**: Use composition like `.ifLet(\.selection, action: \.counter) { EmptyReducer().ifLet(\.value, action: \.self) { Counter() } }` to safely route events to sub-features nested in custom wrappers.
