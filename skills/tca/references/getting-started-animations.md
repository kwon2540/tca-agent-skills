...
This screen demonstrates how changes to application state can drive animations in Composable Architecture.

## Pattern Guidelines
- **Sending with Animations**: You can pass an explicit animation when sending actions to the store (e.g. `store.send(.setColor(color), animation: .linear)`).
- **Binding Animations**: Animate changes made to state through a SwiftUI binding by calling `.animation(_:)` on the binding or store bindings.
- **Asynchronous Animations**: Use `Effect.run` to execute async work and send actions back with animations dynamically.
- **ButtonState Animations**: You can schedule animations on alert buttons, like `.send(.resetConfirmationButtonTapped, animation: .default)`.
