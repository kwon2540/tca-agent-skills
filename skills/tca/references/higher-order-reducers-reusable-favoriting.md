# Higher-Order Reducers - Reusable Favoriting

This screen demonstrates how to construct reusable, generalized feature reducers (like a generic favoriting component) in the Composable Architecture.

## Pattern Guidelines
- **Generic State & Action**: Declare the generic state (e.g. `FavoritingState<ID: Hashable & Sendable>`) and action wrapper independently.
- **Computed State Projections**: Bridge parent feature state fields to the generic child component state using computed properties with custom getters and setters (e.g. `var favorite: FavoritingState<ID> { get { ... } set { ... } }`).
- **Parameterized Reducers**: Define the reusable reducer struct with parameters for passing dependencies or execution closures (e.g. `struct Favoriting<ID> { let favorite: (ID, Bool) async throws -> Bool }`).
- **Reuse in Lists**: Integrate the child component inside list reducers using `Scope` inside a `.forEach` modifier.
