# Getting Started - Alerts & Confirmation Dialogs

This demonstrates how to best handle alerts and confirmation dialogs in the Composable Architecture using `AlertState` and `ConfirmationDialogState`.

## Pattern Guidelines
- **State Representability**: Model alerts/dialogs using `AlertState` and `ConfirmationDialogState` to make them data-driven and easily testable.
- **Presents Wrapper**: Decorate your optional alert/dialog states with `@Presents`.
- **Action Handling**: Nest your presentation actions under `PresentationAction<AlertAction>` inside the parent feature actions.
- **Reducer Integration**: Use the `.ifLet(\.$alert, action: \.alert)` operator inside your reducer body to handle presentation lifecycles correctly.
- **View Presentation**: Scope and bind the presentation in SwiftUI using `.alert($store.scope(\.alert, action: \.alert))`.
