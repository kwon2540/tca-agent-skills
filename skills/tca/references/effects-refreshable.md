...
This application demonstrates how to integrate SwiftUI's pull-to-refresh (`.refreshable`) modifier with Composable Architecture tasks.

## Pattern Guidelines
- **Awaiting Store Tasks**: Tapping or pulling-to-refresh fires an action that returns a task. You can await this task's completion by calling `.finish()` (e.g. `await store.send(.refresh).finish()`).
- **Activity Indicator Control**: SwiftUI's `.refreshable` expects an async block that suspends while loading. By awaiting the store task `.finish()`, you keep the refreshing spinner visible exactly as long as the side effects run in the reducer.
- **Task Cancellation**: You can cancel this in-flight refresh task by sending a cancellation action (e.g. `store.send(.cancelButtonTapped)`).
